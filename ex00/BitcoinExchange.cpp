...
/*   Updated: 2026/02/10 11:22:29 by igngonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::parseInputLine(const std::string &line) {
    ...
    std::map<std::string, double>::iterator it =
        this->database.lower_bound(date);
    double rate = 0.0;
    if (it != this->database.end() && date == it->first)
        rate = it->second;
    else {
        if (it == this->database.begin()) {
            std::cerr << "Error: no earlier rate available for the date "
                      << date << std::endl;
            return;
        }
        --it;
